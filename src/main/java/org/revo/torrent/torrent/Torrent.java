package org.revo.torrent.torrent;

import org.revo.torrent.peer.Peer;
import org.revo.torrent.peer.PeerTcpChannel;
import org.revo.torrent.piece.Piece;
import org.revo.torrent.piece.PieceBlock;
import org.revo.torrent.piece.PieceToFileMapper;
import org.revo.torrent.piece.RequestBlock;
import org.revo.torrent.torrent.torrentdata.FileData;
import org.revo.torrent.torrent.torrentdata.MultiFileInfoData;
import org.revo.torrent.torrent.torrentdata.SingleFileInfoData;
import org.revo.torrent.torrent.torrentdata.TorrentData;
import org.revo.torrent.tracker.TrackerManager;
import org.revo.torrent.util.BitField;
...
public class Torrent {
    ...
    public Torrent(String base,TorrentData torrentData) {
        ...
        if (torrentData.getInfo() instanceof SingleFileInfoData) {
            pieceMapper = new PieceToFileMapper(base,Collections.singletonList(((SingleFileInfoData) torrentData.getInfo()).getFile()));
            ...
            pieceMapper = new PieceToFileMapper(base,((MultiFileInfoData) torrentData.getInfo()).getFiles());
            torrentLength = ((MultiFileInfoData) torrentData.getInfo()).getFiles().stream()
                    .mapToLong(FileData::getLength).sum();
        }
        ...
        Scheduler.submit(() -> {
            ...
            synchronized (peersRequestedBlocks) {
                if (peersRequestedBlocks.containsKey(block)) {
                    ...
                    peersRequestedBlocks.put(block, new HashSet<Peer>() {{
                        add(peer);
                    }});
                    loadBlock = true;
                }
    ...
    public synchronized void peerPieceMessage(Peer peer, PieceBlock block) {
        ...
        peersDownloading.forEach(it -> {
            if (it != peer)
                it.sendCancelBlock(new RequestBlock(block.getPieceIndex(), block.getBaseOffset(), block.getData().length));
        });
