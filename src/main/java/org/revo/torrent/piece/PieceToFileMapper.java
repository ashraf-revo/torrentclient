package org.revo.torrent.piece;
...
public class PieceToFileMapper {
    ...
    public PieceToFileMapper(String base, List<FileData> fileEntryList) {
        this.base = base;
        this.fileEntryList = fileEntryList;

        fileEntryList.forEach(it -> {
            File file = new File(Paths.get(base, it.getName()).toString());
    ...
    public void writePiece(byte[] data, long begin) {
        ...
        while (baseOffset < begin + data.length) { //nullpointerexception
            ...
            if (baseOffset + entry.getLength() > begin && baseOffset <= begin) { //Starting to write
                ...
                try {
                    AsynchronousFileChannel fileChannel = AsynchronousFileChannel.open(Paths.get(base, entry.getName()), StandardOpenOption.CREATE, StandardOpenOption.WRITE);
                ...
                try {
                    fileChannel = AsynchronousFileChannel.open(Paths.get(base, entry.getName()));
    ...
    public void startReadBlock(long begin, int length, Consumer<byte[]> consumer) {
        ...
        while (baseOffset < begin + length) {
            ...
            if (baseOffset + entry.getLength() > begin && baseOffset <= begin) { //Starting to write
                ...
                try {
                    AsynchronousFileChannel fileChannel = AsynchronousFileChannel.open(Paths.get(base, entry.getName()));
